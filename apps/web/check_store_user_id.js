...
async function run() {
    console.log('Checking storefronts.user_id type...');

    const uuid = '00000000-0000-0000-0000-000000000000';
    const { error: uuidError } = await supabase.from('storefronts').select('user_id').eq('user_id', uuid).limit(1);

    if (uuidError && uuidError.message.includes('invalid input syntax for type integer')) {
        console.log('>>> CONFIRMED: storefronts.user_id is INTEGER (WRONG TYPE).');
    } else if (!uuidError) {
        console.log('storefronts.user_id accepts UUID.');
    } else {
